...
(function($) {
    $.fn.bxCarousel = function(options) {
        var defaults = {
            move: 4,				//每次滚动移动图片的数量，默认为4。
            display_num: 4,			//展示图片的数量，默认为4。
            speed: 500,				//图片滚动速度，默认为500毫秒。
            margin: 0,
            auto: false,			//是否自动滚动，默认为false
            auto_interval: 2000,	//当设为自动滚动时，每次滚动的时间间隔（毫秒），默认为2000毫秒即2秒。
            auto_dir: 'next',
            auto_hover: false,		//滚动时，鼠标滑上图片时是否停止滚动，默认为false。
            next_text: 'next',
            next_image: '',
            prev_text: 'prev',
            prev_image: '',
            controls: true 			//是否显示左右滚动按钮图片，默认为true。
        };
